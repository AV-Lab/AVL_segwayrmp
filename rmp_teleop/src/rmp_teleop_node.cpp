...
void RmpTeleop::Initialize()
{
  ...
  m_NodeHandle.param("rotational_velocity_scale", m_RotationalVelocityScale, 0.5);
  m_NodeHandle.param("translational_velocity_boost_scale", m_TranslationalVelocityBoostScale, 4.0);
  ...
  m_VelocityCommandPublisher = m_NodeHandle.advertise<geometry_msgs::Twist>(velocityCommandTopic, 1);
...
void RmpTeleop::ProcessJoystickMessage(const sensor_msgs::Joy::ConstPtr& rpJoyMessage)
{
  ...
  {
    velocityCommand.linear.x = m_TranslationalVelocityBoostScale * m_pJoystickConverter->GetTranslationalVelocity(*rpJoyMessage);
    velocityCommand.angular.z = m_RotationalVelocityBoostScale * m_pJoystickConverter->GetRotationalVelocity(*rpJoyMessage);
  }
  else
  {
    velocityCommand.linear.x = m_TranslationalVelocityScale * m_pJoystickConverter->GetTranslationalVelocity(*rpJoyMessage);
    velocityCommand.angular.z = m_RotationalVelocityScale * m_pJoystickConverter->GetRotationalVelocity(*rpJoyMessage);
  }
