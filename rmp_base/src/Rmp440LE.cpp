...
static const double DEADMAN_VALIDITY_PERIOD = 5.0; // [s]
...
void Rmp440LE::Initialize()
{
  ...
  int portNumber;
  std::string odometryTopic, jointStatesTopic, inertialTopic, pseTopic, motorStatusTopic, batteryTopic, velocityCommandTopic, audioCommandTopic, faultStatusTopic;
  ...
  // m_NodeHandle.param("ip_address", ipAddress, std::string("192.168.1.40"));
  m_NodeHandle.param("ip_address", ipAddress, std::string("10.66.171.5"));
  m_NodeHandle.param("port_number", portNumber, 8080);
  m_NodeHandle.param("device_port", devicePort, std::string("/dev/ttyACM0"));
  m_NodeHandle.param("update_frequency", updateFrequency, 60.0);
  ...
  m_NodeHandle.param("velocity_command_topic", velocityCommandTopic, std::string("/cmd_vel_smooth")); //smoothed vel output
  //m_NodeHandle.param("velocity_command_topic", velocityCommandTopic, std::string("/cmd_vel"));
  ...
  try 
  {
    ...
    m_VelocityCommandSubscriber = m_NodeHandle.subscribe<geometry_msgs::Twist>(velocityCommandTopic, 1, &Rmp440LE::ProcessVelocityCommand, this);
    ...
    ros::Duration maxUpdatePeriod(0);
...
void Rmp440LE::InitializeMessages()
{
  ...
  m_NodeHandle.param("base_frame", baseFrame, std::string("base_footprint"));
  m_NodeHandle.param("odometry_frame", odometryFrame, std::string("odom"));
  m_NodeHandle.param("inertial_frame", inertialFrame, std::string("imu_link"));
  ...
  m_OdometryMsg.pose.pose.orientation = tf::createQuaternionMsgFromYaw(th_);
...
void Rmp440LE::ProcessVelocityCommand(const geometry_msgs::Twist::ConstPtr& rpVelocityCommand)
{
  // if (!IsDeadmanValid())
  // {
  //   return;
  ...
  float normalizedVelocity = static_cast<float>(rpVelocityCommand->linear.x) / maximumVelocity;
  // Segway defines yaw as negative z
  float normalizedYawRate = -1.0 * static_cast<float>(rpVelocityCommand->angular.z) / maximumTurnRate;
  
  if ((normalizedVelocity > fabs(1.0)) || (normalizedYawRate > fabs(1.0)))
  {
    ROS_WARN_STREAM("Velocity command out of range: " << rpVelocityCommand->linear.x << ", " << rpVelocityCommand->angular.z << " should be within: " << maximumVelocity << " [m/s], " << maximumTurnRate << " [rad/s]");
...
void Rmp440LE::UpdateOdometry()  
{
  ...
  double rightfrontvel = static_cast<double>(m_Rmp440LEInterface.GetUserDefinedFeedback<float>(segway::RIGHT_FRONT_VEL_MPS));
  double leftfrontvel = static_cast<double>(m_Rmp440LEInterface.GetUserDefinedFeedback<float>(segway::LEFT_FRONT_VEL_MPS));
  double rightrearvel = static_cast<double>(m_Rmp440LEInterface.GetUserDefinedFeedback<float>(segway::RIGHT_REAR_VEL_MPS));
  double leftrearvel = static_cast<double>(m_Rmp440LEInterface.GetUserDefinedFeedback<float>(segway::LEFT_REAR_VEL_MPS));
  double wheelbase = 0.673595;//static_cast<double>(m_Rmp440LEInterface.GetUserDefinedFeedback<float>(segway::FRAM_WHEEL_TRACK_WIDTH_M));
  ...
  if (m_OdometryInitialized)
  {
    ...
    double d = (deltaLeftSideDisplacement+deltaRightSideDisplacement)/2.0;
    double th = (deltaRightSideDisplacement-deltaLeftSideDisplacement)/1.08616489885305;
    double x =cos(th)*d;
    double y=-sin(th)*d;
    ...
    m_OdometryMsg.pose.pose.position.x = x_;
    m_OdometryMsg.pose.pose.position.y = y_;
    m_OdometryMsg.pose.pose.position.z = z_;
    m_OdometryMsg.pose.pose.orientation = tf::createQuaternionMsgFromYaw(th_);
